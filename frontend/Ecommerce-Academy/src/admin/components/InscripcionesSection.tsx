...
import { INSCRIPCIONES_MOCK, CURSOS_MOCK, ESTUDIANTES_MOCK } from "../mockData";
...
const EMPTY: Omit<Inscripcion, "id" | "estudianteNombre" | "cursoNombre"> = {
  estudianteId: ESTUDIANTES_MOCK[0].id,
  cursoId: CURSOS_MOCK[0].id,
...
}> = ({ inscripciones, onEditar, onEliminar }) => (
  <div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead>
            <tr style={{ background: "#f8f9fa" }}>
              {["ID","Estudiante","Curso","Fecha","Progreso","Estado","Acciones"].map(h => (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <div>
            ...
            <select style={inputStyle} value={form.estudianteId} onChange={e => set("estudianteId", +e.target.value)}>
              {ESTUDIANTES_MOCK.map(e => <option key={e.id} value={e.id}>{e.nombre} {e.apellido}</option>)}
            </select>
          </div>
          <div>
            <label style={labelStyle}>Curso</label>
            <select style={inputStyle} value={form.cursoId} onChange={e => set("cursoId", +e.target.value)}>
              {CURSOS_MOCK.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}
            </select>
          </div>
          <div>
...
const InscripcionesSection: React.FC = () => {
  const [inscripciones, setInscripciones] = useState<Inscripcion[]>(INSCRIPCIONES_MOCK);
  ...
  const resolverNombres = (data: FormData): Pick<Inscripcion, "estudianteNombre" | "cursoNombre"> => {
    const est = ESTUDIANTES_MOCK.find(e => e.id === data.estudianteId);
    const cur = CURSOS_MOCK.find(c => c.id === data.cursoId);
    return {
      estudianteNombre: est ? `${est.nombre} ${est.apellido}` : "",
  ...
  const handleCrear = (data: FormData) => {
    const newId = Math.max(...inscripciones.map(i => i.id)) + 1;
    setInscripciones(p => [...p, { id: newId, ...data, ...resolverNombres(data) }]);
    setTab("listar");
  };

  const handleEditar = (data: FormData) => {
    if (!selected) return;
    setInscripciones(p => p.map(i => i.id === selected.id ? { ...i, ...data, ...resolverNombres(data) } : i));
    setTab("listar");
  };
  ...
  return (
    <div>
      ...
      {tab === "listar"   && <InscripcionesListar inscripciones={inscripciones} onEditar={abrirEditar} onEliminar={abrirEliminar} />}
      {tab === "crear"    && <InscripcionForm modo="crear" inicial={EMPTY} onGuardar={handleCrear} onCancelar={() => setTab("listar")} />}
      {tab === "editar"   && (selected
        ? <InscripcionForm modo="editar" inicial={toFormData(selected)} onGuardar={handleEditar} onCancelar={() => setTab("listar")} />
        : <InscripcionesListar inscripciones={inscripciones} onEditar={abrirEditar} onEliminar={abrirEliminar} />
      )}
      {tab === "eliminar" && (selected
        ? <DeleteConfirm
            ...
            detalle={{ ID: `#${selected.id.toString().padStart(3,"0")}`, Estudiante: selected.estudianteNombre, Curso: selected.cursoNombre, Progreso: `${selected.progreso}%` }}
